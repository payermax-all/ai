...
**Note:** When `integration_mode == drop_in` and GOOGLEPAY/APPLEPAY is selected, the frontend `canMakePayment` call requires additional `subscriptionPlan` and `mitManagementUrl` parameters. See `references/shared/drop-in-frontend.md` section "Subscription-specific: Google Pay canMakePayment parameters" for details.
