...
**Note:** When `integration_mode == drop_in` and GOOGLEPAY or APPLEPAY is selected, the frontend `canMakePayment` call requires `mitManagementUrl` parameter (unlike merchant-manage, `subscriptionPlan` is not required for non-periodic auto debit). See `references/shared/drop-in-frontend.md` section "Subscription-specific: Google Pay canMakePayment parameters" for details.
